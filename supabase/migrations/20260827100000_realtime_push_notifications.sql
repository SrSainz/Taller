-- Keep all open screens synchronized through Postgres Changes and prepare a
-- secure server-side Web Push delivery path for the administrator's devices.
-- The webhook secret is intentionally read from Supabase Vault at runtime;
...
-- These tables feed the open administrator dashboard. RLS continues to decide
-- which rows each Realtime subscriber is allowed to receive.
alter table public.profiles replica identity full;
alter table public.maintenance_reports replica identity full;
alter table public.driver_period_financials replica identity full;
alter table public.commission_reports replica identity full;
...
begin
  ...
  if not exists (
    select 1 from pg_publication_tables
    where pubname = 'supabase_realtime' and schemaname = 'public' and tablename = 'driver_period_financials'
  ) then
    execute 'alter publication supabase_realtime add table public.driver_period_financials';
  end if;

  if not exists (
    select 1 from pg_publication_tables
    where pubname = 'supabase_realtime' and schemaname = 'public' and tablename = 'commission_reports'
  ) then
    execute 'alter publication supabase_realtime add table public.commission_reports';
  end if;
end;
$$;
...
create trigger maintenance_reports_enqueue_app_push
  after insert or update on public.maintenance_reports
  for each row execute function private.enqueue_app_push_notification();
