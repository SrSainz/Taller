-- Avoid evaluating transaction-only fields for driver_entries and other
-- tables. PostgreSQL trigger records expose only the columns of the table
-- that fired the trigger; the old combined condition raised a warning on
-- every driver_entries write because source_document_id does not exist there.
...
declare
  ...
  entry_date date;
begin
  -- A document is the user-visible event. Its central transaction is an
  -- implementation detail of the same upload and must not send a duplicate
  -- notification. Keep this lookup inside the table guard so that trigger
  -- records from driver_entries are never asked for this field.
