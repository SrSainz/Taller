-- Keep refunds extracted from daily billing captures separate from legacy tolls.
alter table if exists public.driver_entries
  add column if not exists refunds numeric not null default 0 check (refunds >= 0);

alter table if exists public.transactions
  drop constraint if exists transactions_type_check;

alter table if exists public.transactions
  add constraint transactions_type_check check (type in ('billing', 'fuel', 'cash', 'tip', 'toll', 'refund', 'wash', 'maintenance', 'miscellaneous'));

-- Rebuild the projection so a billing capture's Reembolsos value reaches the
-- shared daily entry without being confused with a legacy Peajes value.
...
begin
  ...
  loop
    insert into public.driver_entries (driver_id, vehicle_plate, entry_date, billing, cash_collected, tips, fuel_cost, fuel_liters, odometer_km, tolls, refunds, wash_expenses, other_expenses)
    select v_driver, coalesce(v_plate, ''), v_date,
      ...
      coalesce(max((metadata->>'liters')::numeric) filter (where type='fuel' and metadata ? 'liters'),0),
      coalesce(max((metadata->>'odometerKm')::integer) filter (where type='fuel' and metadata ? 'odometerKm'),0),
      coalesce(sum(amount) filter (where type='toll'),0), coalesce(sum(amount) filter (where type='refund'),0),
      coalesce(sum(amount) filter (where type='wash'),0), coalesce(sum(amount) filter (where type='miscellaneous'),0)
    ...
    on conflict (driver_id, entry_date) do update set
      ...
      refunds=excluded.refunds, wash_expenses=excluded.wash_expenses, other_expenses=excluded.other_expenses,
