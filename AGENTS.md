...
- The dashboard formerly called Informes is the default "Vista operativa" home view for the root URL and installed PWA. It opens on General, shows Vista operativa in the top bar, and has no Informes navigation item. The green sidebar is never persistent: navigation opens as an overlay from the top-bar menu, starts with Flota, and its visible brand reads "Vista operativa" and returns home. Nested Gasolina opens the reports workspace on Repostaje.
