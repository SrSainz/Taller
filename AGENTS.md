...
- The dashboard formerly called Informes is the unnamed default home view for the root URL and installed PWA. It opens on General without a visible page title or navigation item. The green sidebar is never persistent: navigation opens as an overlay from the top-bar menu, starts with Flota, and its visible brand reads "Vista operativa" and returns home. Nested Gasolina opens the reports workspace on Repostaje.
- Automatizaciones, Ajustes, and Ayuda never appear in the overlay navigation. They are persistent top-bar shortcuts immediately to the left of the notification bell, with labels on wide screens and icons on compact screens.
- The unnamed home dashboard has exactly four financial KPI cards: Facturación in blue, Mantenimiento in grey, Combustible in red, and Neto in green. Their totals must represent monthly driver billing, workshop spend, recorded fuel spend, and billing minus all vehicle expenses respectively.
