...
- Every destination is a real application window. Flota, Lecturas, Facturas, Mantenimiento, Automatizaciones, Ajustes, and Ayuda must change the route hash, title, content, and active navigation state, but they are entered only from Vista operativa controls.
- There is no sidebar or hamburger menu. Vista operativa > General is the sole navigation hub. Its quick-action row opens Vehículos, Lecturas, Facturas, Mantenimiento, and Combustible; the persistent V button always returns to General.
...
- The dashboard formerly called Informes is the default "Vista operativa" home view for the root URL and installed PWA. It opens on General and shows Vista operativa in the top bar. Gasolina opens the same reports workspace on Repostaje.
- Every root launch is normalized to `#/informes`. A standalone PWA launch always resets to Vista operativa > General even when the previous session closed on another route; direct non-standalone deep links remain available.
...
- Vehicle order is fixed everywhere: 1) 5043 MLC, professional, Toyota Corolla; 2) 5750 MJV, professional, Toyota Corolla; 3) 5754 MJV, professional, Toyota Corolla; 4) 0344 LCP, particular, Lexus IS 300h; 5) 9401 LTG, particular, Toyota Corolla.
