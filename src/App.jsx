...
import { getMaintenanceReportCounts, getMaintenanceReportRecordedAt, getMaintenanceReportStatusLabel, sortMaintenanceReportsByRecordedAt } from "./maintenanceReports";
...
const buildNetExpenseBreakdown = ({ vehicle, fuel, maintenance, commission, periodFactor, driverRows = [], driverNames = [], additionalHistoricalBilling = 0, fuelEntries = [], periodFinancials = [], manualBreakdowns = [], reportMonth = 6, reportYear = 2026 }) => {
  ...
  const configuredInsuranceAmount = getNetMonthlyFixedInsurance(vehicle.plate);
  const insuranceAmount = configuredInsuranceAmount ?? scale(amounts[9]);
  ...
  return [
    ...
    { key: "insurance", label: "Seguro", amount: insuranceAmount, cadence: configuredInsuranceAmount == null ? "Anual" : "Mensual · fijo" },
...
const buildDriverWeeklyRows = (days, entries, manualValues = {}) => {
  ...
  return [
    ...
    { key: "wash", label: "Lavados", values: entries.map((entry, index) => total(entry, "wash", index)) },
    { key: "other", label: "Varios", values: entries.map((entry, index) => total(entry, "other_expenses", index)) },
            ...
            {driverDayDocumentsLoading ? <p className="driver-mobile-calendar-documents__empty">Buscando fotos y archivos del día…</p> : calendarDocuments.length === 0 ? <p className="driver-mobile-calendar-documents__empty"><IconCamera size={16} />Sin fotos de efectivo o repostaje para este día.</p> : <div className="driver-mobile-calendar-documents__list">
              {calendarDocuments.map((document) => {
                ...
                return <article className="driver-mobile-calendar-document" key={document.id}>
                  ...
                  {onDeleteDriverDocument && <button type="button" className="driver-mobile-calendar-document__delete" onClick={() => deleteUploadedDocument(document, deleteKey)} disabled={recordDeleteKey === deleteKey} aria-label={`Borrar ${label.toLowerCase()} ${document.file_name || "archivado"}`}>{recordDeleteKey === deleteKey ? "Borrando…" : "Borrar foto"}</button>}
                </article>;
              })}
            </div>}
          </section>
...
function NetDetailModal({ details, historicalBillingRows: unassignedHistoricalBillingRows = [], periodKey, periodLabel, reportMonth, reportYear, onSelectMonth, onSelectYear, commissionReports = [], commissionReportBusy = false, commissionReportMessage = "", onSaveAlexPayroll, onGenerateAlexReport, onDownloadCommissionReport, onAddExpense, onRemoveExpense, onSaveBreakdown, onClose }) {
  ...
  const handleCollapsedCardKeyDown = (event, plate) => {
    if (event.key !== "Enter" && event.key !== " ") return;
    event.preventDefault();
    toggleVehicle(plate);
  };
...
function VehicleExpenses({ vehicle, transactions = [] }) {
  ...
  const insuranceAmount = getNetMonthlyFixedInsurance(vehicle.plate);
  const leasingAmount = getLeasingAmountForPeriod(vehicle.plate, reportYear, reportMonth);
  const licenseLoanAmount = getLicenseLoanAmountForPeriod(vehicle.plate);
  const inspectionAmount = getAnnualRecurringExpenseAmount("inspection", vehicle.plate, reportMonth);
