       ...
       {driverVehicleOptions.map((vehicle) => {
         ...
         return <section className="admin-accordion" key={vehicle.plate}>
           <button className={`admin-accordion__button${openSection === vehicle.plate ? " admin-accordion__button--open" : ""}`} type="button" onClick={() => toggleSection(vehicle.plate)} aria-expanded={openSection === vehicle.plate} aria-controls={`admin-vehicle-${vehicle.plate.replace(/\s/g, "-")}`}>
             ...
             <span className="admin-accordion__copy"><strong>{vehicle.plate}</strong><span className="admin-accordion__vehicle-subline"><small>{vehicle.model}</small><span className="admin-accordion__status"><i />Activo</span></span><small className="admin-accordion__driver-names" aria-label="Conductores asignados">{vehicleDrivers.length > 0 ? vehicleDrivers.map((driver) => <span key={driver.id}>{driver.full_name}</span>) : <span>Ninguno</span>}</small></span>
             <span className="admin-accordion__meta"><span><IconFileInvoice size={14} /><b>Documentos</b><small>{vehicleDocuments}</small></span></span>
