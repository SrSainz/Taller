  ...
  return (
    <main className={`driver-app driver-mobile-app driver-mobile-app--updated ${preview ? "driver-app--preview" : "driver-app--live"}`}>
      <header className="driver-mobile-topbar">
        {preview && <button type="button" className="driver-mobile-topbar__back app-return-button" onClick={onExitPreview} aria-label="Volver a administración" title="Volver a administración"><IconArrowBackUp size={26} stroke={3.5} /></button>}
...
function MaintenanceView({ initialPlate, invoices, setModal, notify, vehicles, maintenanceSearchSelection, maintenanceReports = [], driverProfiles = [], onSaveMaintenanceReport, onMarkMaintenanceReportReviewed, onOpenMaintenanceReports, onRefreshMaintenanceReports, onDeleteMaintenanceDocument }) {
  ...
  const reportsRefreshRequestRef = useRef(0);
  const workshopVehicle = vehicles.find((vehicle) => vehicle.plate === workshopPlate) ?? vehicles[0];
  ...
  const openMaintenanceReports = async (plate) => {
    ...
    let latestReports = maintenanceReports;
    if (onRefreshMaintenanceReports) {
      ...
      try {
        ...
        if (requestId === reportsRefreshRequestRef.current) setReportsRefreshing(false);
      }
    }
